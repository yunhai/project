...
class Menu extends BasicObject{
	...
	function getCatUrl($module) {
		global $bw;
		return $bw->base_url."{$module}/category/".strtolower(VSFTextCode::removeAccent(str_replace("/", '-', trim($this->title)),'-')). '-' . $this->id.'/';
	}
	...
	function getChildrenLi(){
            ...
            if($this->children){
                foreach($this->children as $obj)
                      $re .= "<li><a href='{$obj->getUrl(0)}' title='{$obj->getTitle()}' target='_blank'><span>{$obj->getTitle()}</span></a></li>";
            }
            return $re;
        }
}
