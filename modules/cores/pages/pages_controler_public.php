...
EOF;
        ...
        $recipient = VSFactory::getSettings()->getSystemKey("email_admin", "mclchuang@gmail.com", "configs");
        $recipientAlias = $this->model->obj->getTitle();

        $from = $bw->input ['email_contacts'];
        $fromName = $bw->input ['email_contacts'];

        $subject = $this->vsLang->getWords("title_email_sender", "Liên hệ");

        $this->sendEmail(compact('message', 'recipient', 'recipientAlias', 'from', 'fromName', 'subject'));
