...
class VSFTemplateEngine {
	...
	function load_template($template_file_name, $strTemplateContent,$cachepath) {
		$this->cache_path=rtrim($cachepath,"/");
...
EOF;';
	...
	function _process_raw_html_foreach_logic($text, $normal_func_data = '', $global_string = "") {
		...
		//----------------------------------------
			...
			$tbpar=explode(" as ", $statement);
			$tbpar=$tbpar[0];
			//----------------------------------------
			// Create PHP statement
			//----------------------------------------
			$function_name = '__foreach_loop__' . str_replace('.', "_", uniqid ( 'id_',true ));
