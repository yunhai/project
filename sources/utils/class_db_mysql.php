...
class db_driver {
    ...
    function query($the_query, $bypass=0) {
        ...
        if (! $this->query_id )
        { 
        	//throw new Exception("11mySQL query error: ". $this->error);
        ...
        if ($this->obj['debug'])
        {
        	...
        	if ( preg_match( "/^(\s*)select/i", $the_query ))
        	{
    ...
    function fatal_error($the_error="")
    {
        ...
        if($_SERVER['REMOTE_ADDR']=='127.0.0.1')
         throw new Exception("mySQL query error: ". $the_error);
        die("");
    }
...
//				$field_values .= $v.",";
//			else{
//				if(strpos($v, 'not_numeric') === 0 && $k == "productCode"){
//					$v = substr($v, 11);
//				}
//				
				$field_values .= "'$v',";
