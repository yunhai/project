		...
		$strResult = "";
                ...
                if(APPLICATION_TYPE=='user'){
                    if($nCurrentPage<$nTotalPage&&$nCurrentPage>1){
                        $strPre = '<a href="'.$this->url.($nCurrentPage-1).$this->p_EndingBy.$bw->input['advance'].'"><</a>';
                        $strNext = '<a href="'.$this->url.($nCurrentPage+1).$this->p_EndingBy.$bw->input['advance'].'">></a>';
                    }
                    else
                        if($nCurrentPage==1){
                            $strPre = '<a href="'.$this->url.$this->text['p_Sub'].'1'.$this->p_EndingBy.$bw->input['advance'].'"><</a>';
                            $strNext = '<a href="'.$this->url.$this->text['p_Sub'].'2'.$this->p_EndingBy.$bw->input['advance'].'">></a>';
                        }
                        if($nCurrentPage==$nTotalPage){
                            $strPre = '<a href="'.$this->url.$this->text['p_Sub'].($nTotalPage-1).$this->p_EndingBy.$bw->input['advance'].'"><</a>';
                            $strNext = '<a href="'.$this->url.$this->text['p_Sub'].($nTotalPage).$this->p_EndingBy.$bw->input['advance'].'">> </a>';
                        }
                }
