...
*/
	function __construct() {
		if($this->categoryField){
			VSFactory::getMenus()->getCategoryGroup($this->categoryName);
		}
		parent::__construct();
	}
		...
		$this->resetResult();
		$this->createMessageSuccess($vsLang->getWords('count_table_success', "count table successfully!"));
		...
		$array=array();
		if(!$DB->simple_exec()) {
			$this->createMessageError($vsLang->getWords('count_table_condition_fail',"There is no item in table!"));
			return $array;
		}
 ...
 */
	function getPageList($url="", $objIndex=3, $size=10, $ajax = 0, $callack=""){
		...
		if($size < $total){
			...
			$pagination->url 				= $ajax?trim($url,'/')."/":$bw->base_url.(trim($url,'/')."/");
...
//			exit();
	...
	function getPageListWithImage($url="", $objIndex=3, $size=10, $ajax = 0, $callack=""){
		global $vsStd,$bw;
		$vsStd->requireFile(LIBS_PATH."/Pagination.class.php");
		$total = $this->getNumberOfObject();
		if($size < $total){
		...
		$option['pageList'] = $this->getObjectsByConditionWithImage();
		$option['total'] = $total;
		return $option;
	}
	function getPageListHash($url="", $objIndex=3, $size=10){
		global $vsStd,$bw;
		$vsStd->requireFile(LIBS_PATH."/Pagination.class.php");
		$total = $this->getNumberOfObject();
		if($size < $total){
			$pagination = new VSFPagination();
			$pagination->ajax				= 0;
			$pagination->callbackobjectId 	= '';
			$pagination->url 				= "#".trim($url,'/')."/";

			$pagination->p_Size 			= $size;
			$pagination->p_TotalRow 		= $total;
			$pagination->SetCurrentPage($objIndex);
			$pagination->BuildPageLinksHash();
	...
	function getPageListHashWidthImage($url="", $objIndex=3, $size=10){
		global $vsStd,$bw;
		$vsStd->requireFile(LIBS_PATH."/Pagination.class.php");
		$total = $this->getNumberOfObject();
		if($size < $total){
