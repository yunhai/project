...
class skin_projects extends skin_objectpublic{
    ...
    <section id="portfolio" class="container">
        <ul class="portfolio-filter">
            <li><a class="btn btn-default  <if="$option['idcate']==0">active</if> " href="#" data-filter="*">{$this->vsLang->getWords("all_cate","All")}</a></li>
            <foreach="$option['cate'] as $value">
            <li><a  class="btn btn-default <if="$option['idcate']==$value->getId()">active</if>   " href="#" data-filter=".cate_{$value->getId()}">{$value->getTitle()}</a></li>
            </foreach>
