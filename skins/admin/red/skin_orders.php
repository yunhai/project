	...
	function viewCart(&$option, &$amount){
		...
		<div class='ui-dialog ui-widget ui-widget-content ui-corner-all'>
        	...
        	<tr align="left">
                ...
                <tr align="left">
                        <th>{$vsLang->getWords('order_location','Khu vực')}:</th>
                        <td>{$option['location']}</td>
                </tr>
                <tr align="left">
                        <th>{$vsLang->getWords('order_shipping','Shipping')}:</th>
                        <td>{$vsLang->getWords('priceunit',' $')}{$option['customer']->getShipping(true)}</td>
                </tr>
                </table>
                 <ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-corner-all-inner ui-widget-header">
                ...
                <if="count($option['cart']) or count($option['gift'])">
                    <table cellspacing="1" cellpadding="1" id='productListTable' width="100%">
                            ...
                            <tbody>
                                    ...
                                    <th>{$vsLang->getWords('priceunit',' $')}{$option['total']}</th>
                                    </tr>
                                    <tr>
                                    <th colspan="6" align="right">
										{$vsLang->getWords('order_shipping','Shipping')}
									</th>
                                    <th>{$vsLang->getWords('priceunit',' $')}{$option['customer']->getShipping(true)}</th>
                                    </tr>
                                    <tr>
                                    <th colspan="6" align="right">
										{$vsLang->getWords('order_product_total','Tổng cộng')}
									</th>
                                    <th>{$vsLang->getWords('priceunit',' $')}{$option['customer']->getTotal(true)}</th>
                                    </tr>
                            </tbody>
                            <tfoot>
                                    <tr>
