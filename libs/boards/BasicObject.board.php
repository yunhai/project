...
class BasicObject {
	...
	public function setAuthor($author) {
        ...
        function getTitle($size=0, $subfix = false) {
		if($size)
			return VSFTextCode::cutString($this->title,$size,$subfix);
		return $this->title;
	}
	...
	function getContent($size=0, $br = 0, $subfix = true, $tags = "") {
		global $vsCom;
		...
		if($size){
			...
			return VSFTextCode::cutString($content, $size, $subfix);
		}
		return $content;
	}
...
//   			$vsPrint->boink_it($exac_url);
	...
	function __destruct(){
        ...
        function convertToObject($object,$tableName="",$doc = 'file') {
                global $vsFile,$DB;
                if ( $object ['fileId'] ){
                        $file = new File();
