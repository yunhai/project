...
class categories {
	...
	public function __construct($name, $num, $crateCategory = true) {
		...
		if ($crateCategory)
			$this->categories = VSFactory::getMenus()->getCategoryGroup ( $this->name );
	}
	...
	public function displayListCategory($buttons, $message = "") {
		...
		if ($this->categories && count ( $this->categories->getChildren () ) > 0)
			VSFactory::getMenus()->buildOptionMenuTree ( $this->categories->getChildren (), &$listCategoryHTML, $liststyle );
	...
	public function addEditCategory($array) {
		...
		$this->setCategories ( $vsMenu->getCategoryGroup ( VSFactory::getModules()->basicObject->getClass () ) );
	...
	function deleteCategory($array) {
		...
		if (count ( $categories )) {
			return $this->displayListCategory ( VSFactory::getLangs ()->getWords ( 'err_category_delete_unempty', "You cannot delete an unempty category! Please delete its children fisrt." ) );
		}
		$vsMenu->menu->__destruct ();
